...
export const fetchPosts = () => {
    ...
    //*therefore the dispatch method will get the request not a plain object as an action
   //refer to lecture 166 of react
    return async (dispatch, getState) => {
        //thunk will intercept the action creator call and pass along the dispatch and getState functions as parameters
        //with thunk we can also return a function as an action like the one below, we can also continue to return plain objects
        const response = await jsonPlaceholder.get('/posts')
        dispatch(
            {
                type: 'FETCH_POSTS',
